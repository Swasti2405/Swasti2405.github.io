...
// Live GitHub stats on project cards: real stars/last-push data instead of
...
document.querySelectorAll('.project-stats[data-repo]').forEach(async (statsEl) => {
  const repo = statsEl.dataset.repo;
  const starEl = statsEl.querySelector('[data-stat="stars"]');
  const updatedEl = statsEl.querySelector('[data-stat="updated"]');
  const contributorsEl = statsEl.querySelector('[data-stat="contributors"]');

  try {
    const res = await fetch(`https://api.github.com/repos/${repo}`);
    if (!res.ok) throw new Error('GitHub API request failed');
    const data = await res.json();
    if (starEl) animateCount(starEl, data.stargazers_count || 0);
    if (updatedEl) updatedEl.textContent = timeAgo(data.pushed_at);

    if (contributorsEl) {
      const contribRes = await fetch(`https://api.github.com/repos/${repo}/contributors?per_page=100`);
      if (contribRes.ok) {
        const contributors = await contribRes.json();
        animateCount(contributorsEl, Array.isArray(contributors) ? contributors.length : 0);
      } else {
        contributorsEl.textContent = 'N/A';
      }
    }
  } catch (err) {
    statsEl.querySelectorAll('[data-stat]').forEach(el => { el.textContent = 'N/A'; });
  }
});
